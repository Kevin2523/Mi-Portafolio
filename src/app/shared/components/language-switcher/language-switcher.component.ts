...
@Component({
  ...
  template: `
    ...
    >
      ...
      <span class="hidden xl:inline leading-none mt-[1px] font-bold">{{ translationService.lang() === 'es' ? 'Español' : 'English' }}</span>
      <span class="xl:hidden leading-none mt-[1px] font-bold">{{ translationService.lang() === 'es' ? 'ES' : 'EN' }}</span>
    </button>
  `
})
